...
def result(request):
    if request.method == 'GET':
        ...
        result = getPrediction(int(name), int(year), float(kilometer), float(mileage), float(engine), float(owner), float(transmission), float(fuel), float(power), float(seats))
        data = {
            "model": request.POST.get('model'),
            "year": int(year),
            "kilometer_driven": float(kilometer),
